...
export const updateMessageContent = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    return await ctx.db.patch("messages", args.messageId, {
      content: args.content,
      status: "completed",
    });
  },
...
export const updateFile = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    return await ctx.db.patch("files", file._id, {
      content: args.content,
      updatedAt: Date.now(),
    });
  },
...
export const renameFile = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (existing) {
      throw new Error(`A ${file.type} name ${args.newName} already exists`);
    }
    ...
    const deleteRecursive = async (fileId: typeof args.fileId) => {
      ...
      if (item.type === "folder") {
        const children = await ctx.db
          .query("files")
          .withIndex("by_project_parent", (query) =>
            query.eq("projectId", item.projectId).eq("parentId", item.parentId),
          )
          .collect();
