...
export const useEditorStore = create<EditorStore>()((set, get) => ({
  ...
  openFile: (projectId, fileId, { pinned }) => {
    ...
    // case 1: opening as preview - whether to replace existing preview or add new
    if (!isOpen && !pinned) {
    ...
    // case 2: opening a tab immediately as pinned
    if (!isOpen) {
      tabs.set(projectId, {
        ...state,
    ...
    // case 3: file is already opened - just activate (and pin if double clicked)
